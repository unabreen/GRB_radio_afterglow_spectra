...
freq_13, freq_49, freq_85, freq_150, flux_total, luminosity = radio_bands(df_030)
...
def compute_spectral_index(
    ...
    flux_col="Flux(mJy)",
    fluxerr_col="Flux uncertainty",
    bin_width=1.0
):
    """
    Compute spectral index alpha(t) by binning in time and
    fitting log F vs log nu in each bin.
    ...
    for i in range(len(bins) - 1):
        ...
        sub = df[(df[time_col] >= t_lo) & (df[time_col] < t_hi)]

        # Require at least two distinct frequencies
        if sub[freq_col].nunique() < 2:
            continue

        # Log quantities
        x = np.log(sub[freq_col].values)
        y = np.log(sub[flux_col].values)

        # Error propagation: sigma_logF = sigma_F / F
        sigma_y = sub[fluxerr_col].values / sub[flux_col].values
...
spix_85_150 = spectral_index(df_030, lower_85, upper_150)    
spix_49_85 = spectral_index(df_030, lower_49, upper_85)
spix_13_49 = spectral_index(df_030, lower_13, upper_49)
...
ax1.set_title('GRB 030329 Flux- 15 GHz')
ax1.set_xlabel('Days')
...
ax2.set_title('GRB 030329 Flux- 8.5 GHz')
ax2.set_xlabel('Days')
...
ax3.set_title('GRB 030329 Flux- 4.9 GHz')
ax3.set_xlabel('Days')
...
ax4.set_title('GRB 030329 Flux- 1.3 GHz')
ax4.set_xlabel('Days')
...
ax1.set_title("GRB 030329- spectral index 8.5-15 GHz")
ax1.set_xscale('log')
ax1.set_xlabel('Days')
...
ax2.set_title("GRB 030329- spectral index 4.9-8.5 GHz")
ax2.set_xscale('log')
ax2.set_xlabel('Days')
...
ax3.set_title("GRB 030329- spectral index 1.3-4.9 GHz")
ax3.set_xscale('log')
ax3.set_xlabel('Days')
